...
def img2label_paths(path, img_paths):
    # Define label paths as a function of image paths
    root_dir = os.path.dirname(path)
    split = os.path.splitext(os.path.basename(path))[0]
    return [f"{root_dir}/labels/{split}/{os.path.splitext(os.path.basename(f))[0]}.txt" for f in img_paths]
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Check cache
        self.label_files = img2label_paths(self.path, self.img_files)  # labels
        ...
        self.label_files = img2label_paths(self.path, cache.keys())  # update
        if single_cls:
...
def extract_boxes(path='../coco/'):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            ...
            # labels
            lb_file = Path(img2label_paths(self.path, [str(im_file)])[0])
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    for i, img in tqdm(zip(indices, files), total=n):
        if not annotated_only or Path(self.path, img2label_paths([str(img)])[0]).exists():  # check label
