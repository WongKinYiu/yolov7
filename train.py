...
def train(hyp, opt, device, tb_writer=None, nested=False, conf_thres: float = 0.001, iou_thres: float = 0.60, overlap: float = 0.5,
    ...
    with mlflow.start_run(run_name=opt.name, nested=nested):
        ...
        if rank in [-1, 0]:
            ...
            run_id = torch.load(weights, map_location=device).get('wandb_id') if weights.endswith(
...
if __name__ == '__main__':
    ...
    # mlflow_client = MlflowClient()  # needed when downloading artefacts

    # Set DDP variables
    opt.world_size = int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1
    opt.global_rank = int(os.environ['RANK']) if 'RANK' in os.environ else -1
    opt.log_artifacts = True  # set log artifacts to true permanently
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            with mlflow.start_run():
                # Train mutation
                results = train(hyp.copy(), opt, device, nested=True, conf_thres=opt.conf_thres, iou_thres=opt.iou_thres,
